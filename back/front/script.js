...
async function displayOmnivoresRecipe() {
    try {
        ...
        if (response.ok) {
            ...
            $("#presentation-recipes").append(`
                <div class="container">
                    <h2>Omnivores</h2>
                    <div class="row" id="recipes-omnivores">
                        <!-- Les recettes seront affichées ici -->
                    </div>
                </div>  
            `);
            for(let i = 0; i < result.length; i++){
                $("#recipes-omnivores").append(`
                    <div class="recipe" data-id=${result[i].id} onclick="showRecipe(this)">
                        <img src=${result[i].imageURL} alt="food" class="food-image">
                        <h2>${result[i].nameFR}</h2>
                    </div>
                `);
            }
        } else {
    ...
    try {
        ...
        if (response.ok) {
            ...
            $("#presentation-recipes").append(`
                <div class="container">
                    <h2>Végétariens</h2>
                    <div class="row" id="recipes-vegetariens">
                        <!-- Les recettes seront affichées ici -->
                    </div>
                </div>
            `);
            for(let i = 0; i < result.length; i++){
                $("#recipes-vegetariens").append(`
                    <div class="recipe" data-id=${result[i].id} onclick="showRecipe(this)">
                        <img src=${result[i].imageURL} alt="food" class="food-image">
                        <h2>${result[i].nameFR}</h2>
                    </div>
    ...
    try {
        ...
        if (response.ok) {
            ...
            $("#presentation-recipes").append(`
                <div class="container">
                    <h2>Vegans</h2>
                    <div class="row" id="recipes-vegans">
                        <!-- Les recettes seront affichées ici -->
                    </div>
                </div>
            `);
            for(let i = 0; i < result.length; i++){
                $("#recipes-vegans").append(`
                    <div class="recipe" data-id=${result[i].id} onclick="showRecipe(this)">
                        <img src=${result[i].imageURL} alt="food" class="food-image">
                        <h2>${result[i].nameFR}</h2>
                    </div>
...
async function consultRecipe(){
    ...
    if (data["nameFR"] == null || data["ingredientsFR"] == null || data["descriptionFR"] == null){
        ...
        $("#auteur").append("<span>Author : </span>"+data["Author"]);
        $("#duree").append("<span>Time : </span>"+formatTime(data["timers"]));
        ...
        $("#auteur").append("<span>Auteur : </span>"+data["Auteur"]);
        $("#duree").append("<span>Durée : </span>"+formatTime(data["timers"]));
...
async function showComments(){
    ...
    try {
        ...
        const response2 = await fetch(`${webServerAddress}/auth/userId`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
        ...
        const id_user = JSON.parse(text2);
                    ...
                    <div class='block-comment${i} col-md-6 mb-4'> 
                        <div class='d-flex justify-content-between align-items-center mb-1'>
                            <div class='fw-bold'>${commentaires[recipeId][i].firstname} ${commentaires[recipeId][i].lastname} ${date} à  ${heure}</div>
                                ${commentaires[recipeId][i].id_user === id_user 
                                    ? `<i class='bi bi-trash btnDeleteComment' role='button' data-timestamp="${commentaires[recipeId][i].timestamp}"></i>`
                                    : ""}
                         </div>
                        <div class="border rounded p-1">${commentaires[recipeId][i].message}</div>
                    </div>`);
            }
